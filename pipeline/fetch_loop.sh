...
Required arguments:
    -p, --pipeline PATH    Path to the pipeline directory containing fetch.sh
    -i, --input PATH      Path to the input data directory

Optional arguments:
    -t, --threads NUM     Number of threads to use (default: 40)
    --paired TRUE/FALSE   Whether the input is paired-end data (default: TRUE)
...
# Validate that pipeline directory exists and contains fetch.sh
if [ ! -d "$pipeline_path" ]; then
    echo "Error: Pipeline directory does not exist: $pipeline_path"
    exit 1
fi

if [ ! -f "$pipeline_path/fetch.sh" ]; then
    echo "Error: fetch.sh not found in pipeline directory: $pipeline_path"
    exit 1
fi
...
for test in "${test_types[@]}"; do
    ...
    for result in "${result_categories[@]}"; do
        ...
        if [ -f "${dir_path}/significant.tsv" ]; then
            ...
            if ! bash "$pipeline_path/fetch.sh" \
