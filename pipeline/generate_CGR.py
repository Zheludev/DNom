...
def process_sequences_parallel(sequences, output_hdf5, output_tsv, num_cpus):
    """Process sequences using multiprocessing and write outputs."""
    total_records = len(sequences)
    
    # Calculate chunk size
    chunk_size = max(100, total_records // (num_cpus * 100))
    sequence_chunks = list(chunks(sequences, chunk_size))
    ...
    with gzip.open(output_tsv, 'wt') as tsv_file:
        ...
        with h5py.File(output_hdf5, 'w') as h5file:
            with tqdm(total=total_records, desc="Processing reads") as pbar:
                for chunk_results in pool.imap(process_chunk, sequence_chunks):
                    for read_id, coords, features in chunk_results:
                        if read_id is not None:
                            ...
                            print(features)  # Print error message
                        pbar.update(1)
...
def main(merge_inpre, r1_inpre, r2_inpre, outpre, strand, num_cpus):
    """Process FASTQ files and generate 3D CGR coordinates and features."""
    num_cpus = min(num_cpus, cpu_count())
    print(f"Processing with {num_cpus} CPUs...")
    
    # Construct file paths
    merge_infile = merge_inpre + '.fastq.gz' if merge_inpre else None
    r1_infile = r1_inpre + '.fastq.gz' if r1_inpre else None
    r2_infile = r2_inpre + '.fastq.gz' if r2_inpre else None
    output_hdf5 = outpre + '.3DCGR.hdf5'
    output_tsv = outpre + '.GCR.tsv.gz'
    ...
    if r1_inpre and r2_inpre:
        ...
        # Concatenate R1 and reverse complemented R2 reads
    ...
    # Process sequences and generate outputs
...
if __name__ == "__main__":
    ...
    parser.add_argument('--r1_inpre', type=str, help='Input R1 file prefix (optional)')
    parser.add_argument('--r2_inpre', type=str, help='Input R2 file prefix (optional)')
    ...
    if not (args.merge_inpre or (args.r1_inpre and args.r2_inpre)):
        parser.error("Either --merge_inpre or both --r1_inpre and --r2_inpre must be provided")
    
    main(args.merge_inpre, args.r1_inpre, args.r2_inpre, args.outpre, args.strand, args.num_cpus)
