...
usage() {
    echo "Usage: $0 -b <base_path> -i <input_path> -p <pipeline_path> -m <metadata_file> -s <strand> -t <num_threads> -o <output_dir>"
...
while getopts "b:i:p:m:s:t:o:h" opt; do
    case $opt in
        b) base_path="$OPTARG" ;;
        i) input_path="$OPTARG" ;;
        p) pipeline="$OPTARG" ;;
...
while IFS=$'\t' read -r SRA description sample strain clone; do
    ...
    python "${pipeline}/generate_CGR.py" \
