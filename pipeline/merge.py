...
def extract_type(path):
    ...
    clustering_dir = os.path.dirname(path)
    parent_dir = os.path.dirname(clustering_dir)
    return os.path.basename(parent_dir)

def apply_prefix_strategy(clustering_df):
    """Apply type-based prefixes to cluster IDs to prevent collisions between types."""
    print("\nApplying prefix strategy to cluster IDs...")
    ...
    # Track how many clusters are renamed for each type
    rename_counts = {}
    
    # Process each type
    for ctype in types:
        # Create a type-specific prefix
        type_prefix = f"{ctype}_"
        
        # Find all non-noise clusters for this type that don't already have the prefix
        mask = (df['type'] == ctype) & (df['clusterID'] != 'noise') & (~df['clusterID'].str.startswith(type_prefix))
        clusters_to_rename = df.loc[mask, 'clusterID'].unique()
        
        # Create rename dictionary for this type
        rename_dict = {cluster_id: f"{type_prefix}{cluster_id}" for cluster_id in clusters_to_rename}
        rename_counts[ctype] = len(rename_dict)
        
        # Apply renaming to the dataframe
        df.loc[mask, 'clusterID'] = df.loc[mask, 'clusterID'].map(rename_dict).fillna(df.loc[mask, 'clusterID'])
    
    # Print summary
    for ctype, count in rename_counts.items():
        print(f"  Renamed {count} clusters for type {ctype} with prefix '{ctype}_'")
...
def find_overlapping_clusters(clustering_df, min_overlap, clustering_paths):
    ...
    # DEBUG - Print sample of the DataFrame
    print("\nDEBUG - Top 5 rows of clustering_df:")
    ...
    # DEBUG - Print counts of noise vs. non-noise clusters
    print("\nDEBUG - Counts of noise vs. non-noise clusters:")
    for ctype in clustering_types:
        noise = len(clustering_df[(clustering_df['type'] == ctype) & 
                               (clustering_df['clusterID'] == 'noise')])
        non_noise = len(clustering_df[(clustering_df['type'] == ctype) & 
                                   (clustering_df['clusterID'] != 'noise')])
        print(f"  {ctype}: {non_noise} non-noise clusters, {noise} noise points")

    # DEBUG - First few cluster IDs from each type
    print("\nDEBUG - First few cluster IDs from each type:")
    for ctype in clustering_types:
        clusters = clustering_df[(clustering_df['type'] == ctype) & 
                              (clustering_df['clusterID'] != 'noise')]['clusterID'].unique()[:5]
        print(f"  {ctype}: {', '.join(map(str, clusters))}")

    # Create mappings
    read_to_cluster = {}
    # CRITICAL FIX: Build cluster_to_type from the unique cluster-type pairs in the DataFrame
    cluster_to_type = {}
    
    # Build the cluster_to_type mapping first, correctly
    print("\nBuilding cluster to type mapping...")
    cluster_type_pairs = clustering_df[clustering_df['clusterID'] != 'noise'][['clusterID', 'type']].drop_duplicates()
    for _, row in cluster_type_pairs.iterrows():
        cluster_to_type[row['clusterID']] = row['type']
    
    # Check the mapping for duplicate clusters with different types
    print("\nDEBUG - Checking for duplicate cluster IDs across types...")
    cluster_counts = clustering_df[clustering_df['clusterID'] != 'noise'].groupby(['clusterID', 'type']).size().reset_index(name='count')
    duplicate_clusters = cluster_counts['clusterID'].value_counts()
    duplicate_clusters = duplicate_clusters[duplicate_clusters > 1]
    if not duplicate_clusters.empty:
        print(f"WARNING: Found {len(duplicate_clusters)} cluster IDs that appear in multiple types!")
        print(duplicate_clusters.head())
        
        # Show some examples
        for cluster_id in duplicate_clusters.index[:5]:
            types = cluster_counts[cluster_counts['clusterID'] == cluster_id]['type'].tolist()
            print(f"  Cluster {cluster_id} appears in types: {types}")
    
    # Initialize cluster_reads
    cluster_reads = {}
    ...
    # Build read mapping and populate cluster_reads
    ...
    # DEBUG - Cluster Type Mapping check
    print("\nDEBUG - Checking cluster type mapping:")
    cgr_count = 0
    rtd_count = 0
    for cluster_id, ctype in list(cluster_to_type.items())[:20]:
        print(f"  Cluster {cluster_id} is type {ctype}")
        if ctype == first_type:
            cgr_count += 1
        elif ctype == second_type:
            rtd_count += 1
    
    print(f"  First {cgr_count + rtd_count} clusters: {cgr_count} are {first_type}, {rtd_count} are {second_type}")

    # DEBUG - First few mappings
    print("\nDEBUG - First few read-to-cluster mappings:")
    read_count = 0
    for read_id, clusters in read_to_cluster.items():
        if read_count < 5:
            print(f"  Read {read_id} is in clusters: {clusters}")
            read_count += 1

    # Find clusters for each type using the corrected mapping
    ...
    # DEBUG - Print sample of clusters from each type
    print(f"\nDEBUG - First 5 clusters from {first_type}:")
    print(list(first_clusters)[:5] if first_clusters else "No clusters found")
    print(f"\nDEBUG - First 5 clusters from {second_type}:")
    print(list(second_clusters)[:5] if second_clusters else "No clusters found")
    
    # Print cluster counts to verify they match the initial counts
    print(f"\nDEBUG - Cluster counts from mapping:")
    print(f"  {first_type}: {len(first_clusters)} clusters (expected {initial_counts[first_type]})")
    print(f"  {second_type}: {len(second_clusters)} clusters (expected {initial_counts[second_type]})")
    ...
    for cluster1 in tqdm(first_clusters):
        ...
        # Check if cluster1 actually has reads
        if not reads1:
            print(f"WARNING: Cluster {cluster1} has no reads")
            continue
        ...
        for cluster2 in second_clusters:
            ...
            # Check if cluster2 actually has reads
            if not reads2:
                print(f"WARNING: Cluster {cluster2} has no reads")
                continue
    ...
    # DEBUG - Print distribution of overlaps
    if overlap_counts:
        print("\nDEBUG - Overlap statistics:")
        print(f"  Total pairs with any overlap: {len(overlap_counts)}")
        print(f"  Min overlap: {min(overlap_counts)}")
        print(f"  Max overlap: {max(overlap_counts)}")
        print(f"  Average overlap: {sum(overlap_counts)/len(overlap_counts):.2f}")
        print(f"  Overlaps >= {min_overlap}: {sum(1 for o in overlap_counts if o >= min_overlap)}")
...
def merge_clusters(clustering_df, volume_df, overlapping_pairs):
    ...
    for i, group in tqdm(enumerate(merged_groups), total=len(merged_groups)):
        # Create a mapping from old to new ids for all clusters in this group
        group_volumes = 0
        group_components = 0
        
        for cluster in group:
            # Try to find the cluster in volume_df - accounting for possible prefixes
            # First try exact match
            matches = volume_df[volume_df['clusterID'] == cluster]
            
            # If no exact match, try to find by checking if any row ends with the cluster ID
            # (in case we're dealing with prefixed IDs like 'CGR_c_123')
            if len(matches) == 0:
                # Strip any potential prefix (like 'CGR_' or 'RTD_')
                if '_' in cluster:
                    base_cluster_id = cluster.split('_', 1)[1]  # Get part after first underscore
                    # Look for entries with this base ID
                    base_matches = volume_df[volume_df['clusterID'].str.endswith(base_cluster_id)]
                    if len(base_matches) > 0:
                        matches = base_matches
            
            # If we found any matches, add their volumes and components
            if len(matches) > 0:
                for _, row in matches.iterrows():
                    group_volumes += row['estVol']
                    group_components += row['components']
            else:
                print(f"Warning: No volume information found for cluster {cluster}")
        
        new_volumes.append({
            'clusterID': f'm_{i}',
            'estVol': group_volumes,
    ...
    for i, cluster in tqdm(enumerate(singleton_clusters), total=len(singleton_clusters)):
        ...
        # If we found any matches, use the first one (there should be only one)
        if len(matches) > 0:
            ...
            print(f"Warning: No volume information found for singleton cluster {cluster}")
...
def print_summary(clustering_paths, clustering_df, volume_df, overlap, overlapping_pairs, iteration_count):
...
def main(clustering_paths, volume_paths, overlap, output_clustering, output_volume, output_otu):
    ...
    # Track the number of overlapping pairs in each iteration to detect convergence
    previous_overlap_count = -1

    while True:
        iteration_count += 1
        print(f"\n======== ITERATION {iteration_count} ========")
        
        # Apply prefix strategy at the beginning of each iteration
        clustering_df = apply_prefix_strategy(clustering_df)
        
        # Find overlapping clusters
        overlapping_pairs = find_overlapping_clusters(clustering_df, overlap, clustering_paths)
        
        # Check if we have no overlapping pairs or if the number of overlapping pairs hasn't changed
        if not overlapping_pairs:
            print(f"No more overlapping clusters found after {iteration_count} iterations.")
            break
        
        # Check for convergence - if the number of overlapping pairs hasn't changed
        current_overlap_count = len(overlapping_pairs)
        if current_overlap_count == previous_overlap_count:
            print(f"Convergence detected: Found {current_overlap_count} overlapping pairs in consecutive iterations.")
            print(f"Stopping after {iteration_count} iterations as no further progress is being made.")
            break
        
        # Update for the next iteration
        previous_overlap_count = current_overlap_count
    ...
    # Print summary
    print_summary(clustering_paths, clustering_df, volume_df, overlap, all_overlapping_pairs, iteration_count)
